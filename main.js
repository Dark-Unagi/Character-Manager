...
function updateEquipmentDisplay(character) {
    ...
    character.equipment.forEach((item, index) => {
         const restriction = getItemQuantityLimit(item.name);
         const atLimit = restriction && item.quantity >= restriction.limit;
         const limitIndicator = restriction
             ? `<span class="text-xs ${atLimit ? 'text-orange-500' : 'text-gray-400 dark:text-gray-500'}" title="Max ${restriction.limit}">(/${restriction.limit})</span>`
             : '';
        ...
        itemDiv.innerHTML = `
                    ...
                    <div class="flex items-center space-x-2">
                        <span class="text-sm dark:text-white text-black">x${item.quantity}</span>${limitIndicator}
...
// =================================================
// EQUIPMENT QUANTITY LIMITS (keyword-based)
// =================================================
// Each keyword is checked case-insensitively against item names.
// If an item name contains the keyword, its quantity is capped at the limit.
// Add or remove entries here to manage restricted items.
const EQUIPMENT_QUANTITY_LIMITS = {
    'torch':    6,
    'oil':      6,
    'fuel':     6,
    'candle':   6,
...
// Check if an item name matches a restricted keyword and return the limit (or null)
function getItemQuantityLimit(itemName) {
    const lowerName = itemName.toLowerCase();
    for (const [keyword, limit] of Object.entries(EQUIPMENT_QUANTITY_LIMITS)) {
...
function showEquipmentQuantityModal(itemIndex) {
    ...
    const restriction = getItemQuantityLimit(item.name);
    const maxQty = restriction ? restriction.limit : 99;

    // Build restriction notice if applicable
    const restrictionNotice = restriction
        ? `<div class="text-xs text-orange-600 dark:text-orange-400 mt-2"><i class="fas fa-exclamation-triangle mr-1"></i>Limit: max ${restriction.limit} (${restriction.keyword})</div>`
        : '';
    ...
    modal.innerHTML = `
                <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg max-w-sm w-full mx-4">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" id="newQuantity" class="w-full px-3 py-2 text-base border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:ring-2 focus:ring-primary focus:border-transparent" value="${item.quantity}" min="1" max="${maxQty}">
                        ${restrictionNotice}
                    </div>
                    <div class="flex justify-end space-x-3">
    ...
    // Shared update logic with limit enforcement
     function applyQuantityUpdate() {
         let newQuantity = parseInt(input.value);
         if (!newQuantity || newQuantity < 1) { modal.remove(); return; }
 
         if (restriction && newQuantity > restriction.limit) {
             newQuantity = restriction.limit;
             showCustomDialog('Quantity Limit', `"${item.name}" is restricted to a maximum of ${restriction.limit}.`);
         }
        ...
        modal.querySelector('.confirm-quantity').addEventListener('click', applyQuantityUpdate);
...
document.getElementById('equipmentForm').addEventListener('submit', function (e) {
    ...
    if (currentCharacter) {
        ...
        const itemName = document.getElementById('equipmentName').value;
         let itemQuantity = parseInt(document.getElementById('equipmentQuantity').value);
 
         // Enforce keyword-based quantity limit on new items
         const restriction = getItemQuantityLimit(itemName);
         if (restriction && itemQuantity > restriction.limit) {
             itemQuantity = restriction.limit;
             showCustomDialog('Quantity Limit', `"${itemName}" is restricted to a maximum of ${restriction.limit}.`);
         }
        const item = {
            name: itemName,
...
const SPELL_DAMAGE_DATA = {
    'acid/poison spray': { dice: '1d6', note: 'acid or poison damage', usesProficiency: true },
    'bolt': { dice: '1d10', note: '', usesProficiency: true },
    'burning hands': { dice: '1d8', note: 'fire damage', usesProficiency: true },
    'call lightning': { dice: '1d6', note: 'lightning damage', usesProficiency: true },
    'chain lightning': { dice: '1d8', note: 'lightning damage', usesProficiency: true },
    'chill touch': { dice: '1d4', note: 'cold damage', usesProficiency: false },
    'chromatic orb': { dice: '1d6', note: '', usesProficiency: true },
    'cloud kill': { dice: '1d6', note: 'poison damage', usesProficiency: true },
    'eldritch blast': { dice: '1d6', note: 'per bolt', usesProficiency: false },
    'feeblemind': { dice: '1d4', note: 'psychic damage', usesProficiency: true },
    'fireball': { dice: '1d6', note: '', usesProficiency: true },
    'fire storm': { dice: '1d10', note: 'fire damage', usesProficiency: true },
    'flame strike': { dice: '1d8', note: 'fire and radiant damage', usesProficiency: true },
    'inflict wounds': { dice: '2d6', note: '', usesProficiency: true },
    'lightning bolt': { dice: '1d10', note: 'lightning damage', usesProficiency: true },
    'magic missile': { dice: '1d4', note: '+ Modifier (Max damage)', usesProficiency: false },
    'moonbeam': { dice: '1d10', note: '', usesProficiency: true },
    'scorching ray': { dice: '1d8', note: 'per bolt', usesProficiency: false },
    'shatter': { dice: '1d8', note: 'psychic', usesProficiency: true },
    'shocking grasp': { dice: '1d8', note: 'lightning damage', usesProficiency: true },
    'sleep': { dice: '2d4', note: '', usesProficiency: false },
    'spiritual weapon': { dice: '1d6', note: 'per Spell Point used', usesProficiency: false },
    'sunbeam': { dice: '1d8', note: '', usesProficiency: true },
    'vampiric touch': { dice: '1d8', note: 'heals caster', usesProficiency: true },
    'wall of fire': { dice: '1d10', note: 'fire damage', usesProficiency: true },
    'wall of ice': { dice: '1d6', note: 'cold damage', usesProficiency: true },

        // Healing spells — isHealing: true skips Spell Attack Bonus and Boon
    'cure wounds': { dice: '1d4', note: 'recover HP', usesProficiency: false, isHealing: true },
    'mass cure wounds': { dice: '2d4', note: 'recover HP', usesProficiency: false, isHealing: true },
    'prayer of healing': { dice: '2d4', note: 'recover HP or Stress', usesProficiency: false, isHealing: true },
    'regenerate': { dice: '1d4', note: 'recover HP', usesProficiency: false, isHealing: true },
};
        ...
        group.spells.forEach(spell => {
            ...
            if (SPELL_DAMAGE_DATA[lowerName] && !seenNames.has(lowerName)) {
                ...
                damageSpells.push({
                    ...
                    isHealing: data.isHealing || false
                });
            }
        });
    });
...
function showSpellDamageSelectionModal() {
    ...
    function buildSpellRowHTML(spell, index) {
        ...
        const rollBtnClass = isHealing
             ? 'bg-green-500 hover:bg-green-600'
             : 'bg-red-500 hover:bg-red-600';
         const borderClass = isHealing
             ? 'border-green-300 dark:border-green-700'
             : 'border-gray-300 dark:border-gray-600';
         const healingTag = isHealing
             ? '<span class="text-xs text-green-500 dark:text-green-400 font-semibold ml-1">Heal</span>'
             : '';

        return `
          <div class="flex items-center justify-between p-3 border ${borderClass} rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors" data-spell-row="${index}">
              ...
              <div class="flex-1 mr-3 min-w-0">
                 <div class="font-semibold text-sm">${spell.name}${healingTag}</div>
             ...
             <button class="spell-damage-roll-btn px-3 py-2 ${rollBtnClass} text-white text-sm font-semibold rounded-lg transition-colors whitespace-nowrap" data-spell-index="${index}">
                 Roll
             </button>
         </div>`;
    }
...
function rollSpellDamage(spell) {
    ...
    // Healing spells do NOT use Spell Attack Bonus
     ...
     if (!isHealing) {
         ...
         spellModifier = dcSave !== null ? dcSave - 8 : 0;
         spellAbility = getSpellcastingAbility();
     }
 
     // Boon damage bonus (per die, same as weapons) — not applied to healing
    ...
    // Build the results breakdown: [rolls] + modifier (if applicable)
    ...
    resultModal.innerHTML = `
         <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg max-w-sm w-full mx-4">
             ...
             <p class="text-sm ${isHealing ? 'text-green-500 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'} text-center mb-4">${isHealing ? 'Healing Roll' : 'Spell Damage Roll'}</p>
 
             <div class="text-center mb-4">
                 ${formulaHTML}
                 <div class="damage-rolls-display text-lg mb-2">${breakdownHTML}</div>
                 <div class="text-3xl font-bold ${isHealing ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}">${grandTotal}</div>
                 ${noteHTML}
                 ${modifierDetailHTML}
             </div>
...
function showWeaponDamageResults(weaponName, rollResults, abilityModifier, weaponType = 'primary', existingResults = null) {
    ...
    function formatFlatBonuses(r, abilityMod) {
        ...
        if (r.boonDamageTotalBonus)
            parts.push(`+${r.boonDamageTotalBonus} Boon`);
            ...
            return `
                    ...
                    : `${r.damageType}`
                            ...
                            <div class="damage-rolls-display text-lg mb-1">
                                ...
                                ${(r.boonDamageTotalBonus || 0) > 0 ? ` + ${r.boonDamageTotalBonus} Boon (+${r.boonDamagePerDie}/die)` : ''}
                                + ${abilityMod} mod
                            </div>
...
function rerollWeaponDamage() {
    ...
    const rerollSet = (dataSet) => {
        return dataSet.map(data => {
            ...
            // Recalculate boon total bonus (per die × total dice)
            const boonPerDie = data.boonDamagePerDie || 0;
            const boonTotalBonus = boonPerDie * data.totalDice;
            ...
            return {
                ...
                twoHandedTotalBonus,
                boonDamagePerDie: boonPerDie,
                boonDamageTotalBonus: boonTotalBonus
            };
        });
    };
